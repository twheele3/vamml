from . import utils
__all__ = ['Experiment','collate_metadata', 'make_dataset','utils','build_model']
...
from tensorflow.keras.layers import Activation,BatchNormalization,Concatenate,Conv2D,\
    Conv2DTranspose,Dense,Dropout,Input,MaxPooling2D,Reshape
from tensorflow.keras import Model,Layer

from .utils import shapes,images,alignments,voxel_formats
from .utils.json import load_json,save_json
from .utils.default_pars import default_pars
from .utils.dataset import get_examples
from .utils.augments import standard_augs
...
class Experiment:
    def __init__(self,
        ...
        Args:
            ...
            - template (str)  : Loads template parameters from specified file location. 
                                ...
                                If str, tries to update default parameters from specified json file. 
    ...
    def generate_shapes(self,
        ...
        assert type(batch_size) is int,'batch_size is not integer'
    ...
    def get_calibration(self,
        ...
        # Tries to autodetect
            ...
            assert len(candidates) == 1,f'Too many candidate calibration images found: {candidates}'
    ...
    def export_voxels(self) -> None:
        ...
        # Loading arrays
        assert hasattr(self,'base_shapes'),"base_shapes not found. Has generate_shapes been run yet?"
    ...
    def export_aligned(self, 
        ...
        with alive_bar(len(to_process), title='Exporting data') as bar:
            for i in to_process:
                ...
                img2 = alignments.align_affine_trs(img)
                # Unpacking to re-pack to array pkl later. 
                # Doing after relative since that has a potential processing step.
                to_package[i] = [np.array(img),np.array(img2)]  
                bar()
        if len(to_process) > 0:
            packaged,prel = [],[]
            # Getting filler for entries to skip
            zero_template = np.zeros_like(np.array(img))
            for i in range(len(self.image_features)):
                if i in self.pars['skip_images']:
                    packaged.append(zero_template[...,:2])
                    prel.append(zero_template[...,:2])
                else:
                    packaged.append(to_package[i][0][...,:2])
                    prel.append(to_package[i][1][...,:2])
    ...
    def metadata(self,
                 metadata: dict, 
                 replace: bool = False):
        """Updates metadata parameters with provided dictionary. If replace is True,
        removes any previous metadata points."""
        assert type(metadata) is dict,"metadata arg is not dictionary."
        if replace:
            self.pars['metadata'] = metadata
        else:
...
def collate_metadata(base_dir: str,
    ...
    Args:
        ...
        include_relative (bool) : Whether to include relative-aligned outputs. Default false.
    """
    #TODO: normpath base_dir = os.path.normpath(base_dir)
    if out_csv is None:
            ...
            for i in range(pars['batch_size']):
                if i not in pars['skip_images']:
                    ...
                    if not include_relative:
                        outputs = [i for i in outputs if 'relative' not in i]
                    for arr in outputs:
                        r = {}
                        r['source'] = os.path.join(d,arr)
                        r['expt_index'] = i
                        r['base_image'] = os.path.join(d,
                                                       os.path.join('images',
                                                                    pars['image_dict'][i]))
                        r['array_size'] = pars['array_size']
                        r.update(meta_pars)
                        # Adding relative pointer for scaling discrimination.
                        if 'relative' in arr:
                            r['relative'] = True
    ...
    pd.DataFrame.from_records(records).to_csv('.'.join([os.path.splitext(out_csv)[0],
                                                        'csv']),index=False)

def make_dataset(
        out_dir: str,
        metadata: str,
        metadata_cols: list = ['dose','voxel_size','z_height'],
        splits: dict[str,list[int]]|None = None,
        ) -> None:
    """Compiles tensorflow dataset from VAMML exeriments. 
    
    Args:
        out_dir (str) : Output location of tensorflow dataset. Infers dataset name from lowest level
        metadata (str) : Pointer to either a csv or directory. If directory, indicates head directory 
                         from which to compile experiments. if csv, metadata csv of experiment datapoints 
                         to compile as assembled from vamml.collate_metadata for info
        metadata_cols (list) : A list of column names in compile_csv to parse to tensorflow.
        splits (dict[str,list[int]]|None) : A dictionary of train/test/validation splits and indices for each.
                                            All indices in compile_csv must be accounted for. If None,
                                            defines a single split of 'train'
    """
    out_dir = os.path.normpath(out_dir)
    # Assertion checks
    assert (os.path.exists(metadata)),"Metadata specified does not exist: {metadata}"
    # Loading or creating metadata csv
    if os.path.isfile(metadata):
        df = pd.read_csv(metadata)
    else:
        base_dir = os.path.normpath(metadata)
        collate_metadata(base_dir)
        df = pd.read_csv(os.path.join(base_dir,'metadata.csv'))
    # Asserting all indices are accounted for in splits.
    if splits is not None:
        indices = []
        for k in splits.keys():
            indices.extend(splits[k])
        assert np.all([i in range(len(df)) for i in indices] + 
                      [i in indices for i in range(len(df))]),\
            "Indices in splits and compiled dataframe do not match."
    # Forcing dose value into df/metadata,putting as first column.
    if 'dose' not in df.columns:
        df['dose'] = 0.
        warnings.warn('No "dose" column found, inserting placeholder value.')
    if 'dose' in metadata_cols:
        metadata_cols.remove('dose')
    metadata_cols.insert(0,'dose')
    # Getting median array size for casting/resizing, using median in case multiple.
    size = int(df['array_size'].median())
    if df['array_size'].median() != df['array_size'].mean():
        warnings.warn(f'Uneven array sizes found! Using most common size of {size}')
    # Storing categorical variable keys for later model calls.
    catkey = {}
    # Converting non-numeric to categorical-numeric encoding.
    for c in metadata_cols:
        assert c in df.columns,f'Column "{c}" not found in metadata_csv'
        if not pd.api.types.is_numeric_dtype(df[c]):
            df[c] = df[c].astype('category')
            l = ['nan']
            l.extend(df[c].cat.categories.to_list())
            # Pandas encodes NaNs in categorical as -1, so re-encoding as 0.
            df[c] = df[c].cat.codes + 1.
            catkey[c] = dict(zip(l,range(len(l))))
        # Converting any other numeric columns to float if necessary
        else:
            catkey[c] = 'float'
            df[c] = df[c].astype('float')
            df[c] = df[c].fillna(0)
    # Adding utility values to metadata_cols to locate source arrays.
    metadata_cols.extend(['source','expt_index'])
    examples = get_examples(df[metadata_cols], 
                            size = int(df['array_size'].median()))
    record_name = [i for i in out_dir.split(os.path.sep) if i != ''][-1]
    par_docstring = ''.join(['Encoded experiment parameters: ',str(catkey)])
    featuredict = {
        "image": tfds.features.Tensor(shape=(size, size, 1), 
                                      dtype=tf.float32, doc="Extracted feature from gel image"),
        "label": tfds.features.Tensor(shape=(size, size, 1), 
                                      dtype=tf.float32, doc="Print array"),
        "parameters" : tfds.features.Tensor(shape=(len(metadata_cols)-2,), 
                                            dtype=tf.float32, doc=par_docstring),
        }
    features = tfds.features.FeaturesDict(featuredict)
    if splits is None:
        splits = {'train':[i for i in range(len(df))]}
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    for split in splits.keys():
        # Single shard tfrecord due to manageable data size
        with tf.io.TFRecordWriter(
            os.path.join(out_dir, f'{record_name}-{split}.tfrecord-00000-of-00001')
        ) as writer:
            for i in splits[split]:
                    example = examples[i]
                    writer.write(example.SerializeToString())
    split_info = tfds.folder_dataset.compute_split_info(
        filename_template=tfds.core.ShardedFileTemplate(Path(out_dir)),
        out_dir=out_dir,
    )
    print(features)
    tfds.folder_dataset.write_metadata(
        ...
        supervised_keys=(("image", "parameters"), ("label",)),
    )


def build_model(imageSize = (256,256), 
                metadataSize = 6,
                dropoutRate = 0.15, 
...
class Augments(Layer):
    ...
    def __call__(self,inputs,output):
        ...
        aug_vals = tf.concat([dose,other],-1)
        # Re-layering input with augmented output
        concat_img = tf.concat([input_img, output_aug1], axis=-1)
