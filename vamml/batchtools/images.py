...
def find_grid_dist(image: str|npt.NDArray,
    ...
    args:
        im (str,arr[N,M]) : An image or 2D array of an image of a calibration grid
        plot (bool)       : Whether to plot proof in-line

    returns: 
        float : unit/px with respect to grid unit distance
    """
    if type(image)==str:
...
def process_coomassie(image_name,cal_size,tophat=False,**kwargs):
    ...
    if tophat is not False:
        mask = white_tophat(hsv[...,1:].prod(-1),
                            footprint=np.ones((int(tophat/cal_size),int(tophat/cal_size))))
    else:
        mask = hsv[...,2]
    mask = mask > (mask.max() * 0.5)
    # Creating mask to cover up small noisy features in image (fibers, particles, etc)
    mask = binary_dilation(binary_erosion(mask,np.ones((int(0.1/cal_size),int(0.1/cal_size)))),
                           np.ones((int(0.1/cal_size),int(0.1/cal_size))))
